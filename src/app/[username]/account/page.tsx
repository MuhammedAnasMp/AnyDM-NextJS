...
export default function CustomerAccountPage({ params }: PageProps) {
  ...
  const storeOwnerDmUrl = `https://ig.me/m/${targetIgHandle}`;
  ...
  const completedOrdersCount = allOrders.filter(
    (o) => ["DELIVERED", "COMPLETED", "CONFIRMED", "PAID"].includes((o.order_status || "").toUpperCase())
  ).length;
  ...
  return (
    <div className={cn("min-h-screen flex flex-col justify-between transition-colors duration-300 font-sans antialiased text-xs sm:text-sm", styles.bodyClass, styles.fontBody)}>
      ...
      <header className={cn("sticky top-0 z-50 border-b backdrop-blur-md transition-colors shadow-2xs", styles.navClass, styles.dividerClass)}>
        <div className="mx-auto flex h-13 max-w-7xl items-center justify-between px-3.5 sm:px-5">
          ...
          <button
            ...
            className={cn(
              "md:hidden flex h-8 w-8 items-center justify-center rounded-lg border transition-all shrink-0 cursor-pointer",
      ...
      ========================================== */}
      <main className="mx-auto max-w-7xl px-3.5 py-4 sm:px-5 sm:py-5 space-y-3.5 flex-1 w-full min-h-[70vh]">
        ...
        <section className="grid grid-cols-3 gap-2 sm:gap-3">
          ...
          {/* Card 1: Total Orders */}
          <div className={cn("group p-2 sm:p-2.5 px-2.5 sm:px-3 rounded-lg border shadow-2xs transition-all hover:border-current/20 flex flex-col sm:flex-row sm:items-center justify-between gap-1.5 sm:gap-2 min-w-0", styles.cardClass)}>
            <div className="flex items-center gap-2 min-w-0">
              <div className="w-6 h-6 sm:w-6.5 sm:h-6.5 rounded-md bg-black/5 dark:bg-white/5 border border-black/10 dark:border-white/10 text-current opacity-75 group-hover:bg-sky-500/10 group-hover:border-sky-500/20 group-hover:text-sky-500 group-hover:opacity-100 flex items-center justify-center shrink-0 transition-all">
                <Package className="w-3 h-3 sm:w-3.5 sm:h-3.5" />
              </div>
              <div className="min-w-0">
                <div className={cn("text-[10px] sm:text-[11px] font-semibold truncate leading-tight", styles.textColorClass)}>
                  Total Orders
                </div>
            ...
            <div className={cn("text-sm sm:text-lg font-extrabold shrink-0 tracking-tight self-start sm:self-auto", styles.fontHeadline, styles.textColorClass)}>
              {allOrders.length}
            </div>
          </div>
          ...
          <div className={cn("group p-2 sm:p-2.5 px-2.5 sm:px-3 rounded-lg border shadow-2xs transition-all hover:border-current/20 flex flex-col sm:flex-row sm:items-center justify-between gap-1.5 sm:gap-2 min-w-0", styles.cardClass)}>
            <div className="flex items-center gap-2 min-w-0">
              <div className="w-6 h-6 sm:w-6.5 sm:h-6.5 rounded-md bg-black/5 dark:bg-white/5 border border-black/10 dark:border-white/10 text-current opacity-75 group-hover:bg-amber-500/10 group-hover:border-amber-500/20 group-hover:text-amber-500 group-hover:opacity-100 flex items-center justify-center shrink-0 transition-all">
            ...
            <div className={cn("text-sm sm:text-lg font-extrabold shrink-0 tracking-tight text-amber-500 self-start sm:self-auto", styles.fontHeadline)}>
              {activeOrdersCount}
            </div>
          </div>
          ...
          <div className={cn("group p-2 sm:p-2.5 px-2.5 sm:px-3 rounded-lg border shadow-2xs transition-all hover:border-current/20 flex flex-col sm:flex-row sm:items-center justify-between gap-1.5 sm:gap-2 min-w-0", styles.cardClass)}>
            <div className="flex items-center gap-2 min-w-0">
              <div className="w-6 h-6 sm:w-6.5 sm:h-6.5 rounded-md bg-black/5 dark:bg-white/5 border border-black/10 dark:border-white/10 text-current opacity-75 group-hover:bg-black/10 dark:group-hover:bg-white/10 group-hover:opacity-100 flex items-center justify-center shrink-0 transition-all">
        ...
        <div className="grid grid-cols-1 gap-3.5 lg:grid-cols-12">
                  ...
                  <>
                    ...
                    <div className="mb-3.5 flex items-start justify-between gap-3">
                      <div className="flex min-w-0 items-start gap-2.5">
                        <div className="w-8 h-8 rounded-md bg-black/5 dark:bg-white/5 border border-black/10 dark:border-white/10 text-current opacity-75 hover:bg-sky-500/10 hover:border-sky-500/20 hover:text-sky-500 hover:opacity-100 flex items-center justify-center shrink-0 transition-all">
                    ...
                    <div className={cn("mb-3.5 grid grid-cols-2 sm:grid-cols-3 gap-px overflow-hidden rounded-md border", styles.dividerClass)}>
                      <div className="bg-black/5 dark:bg-white/5 p-2.5">
                        ...
                        <p className={cn("text-[11px] font-semibold opacity-90", styles.textColorClass)}>Received</p>
                      </div>
                      <div className="bg-black/5 dark:bg-white/5 p-2.5">
                        <p className={cn("text-[9.5px] opacity-60 mb-0.5", styles.textMutedClass)}>Date</p>
                      ...
                      <div className="bg-black/5 dark:bg-white/5 p-2.5 col-span-2 sm:col-span-1">
                        <p className={cn("text-[9.5px] opacity-60 mb-0.5", styles.textMutedClass)}>Type</p>
                    ...
                    <div className="mb-3.5 space-y-1.5">
                      <div className="flex items-center justify-between text-xs">
                        <span className={cn("font-medium opacity-75 text-[11px]", styles.textColorClass)}>Order status</span>
                      ...
                      <div className={cn("flex justify-between text-[9.5px] opacity-60 pt-0.5 font-medium", styles.textMutedClass)}>
                        ...
                        <span className={cn("font-semibold opacity-100", styles.textColorClass)}>Completed</span>
                      </div>
                    </div>
          ...
          <aside className="min-w-0 space-y-3.5 lg:col-span-4">
            ...
            <section className={cn("overflow-hidden rounded-lg border p-3.5 sm:p-4 shadow-2xs space-y-2.5", styles.cardClass)}>
              <div className="w-7.5 h-7.5 rounded-md bg-black/5 dark:bg-white/5 border border-black/10 dark:border-white/10 text-current opacity-75 hover:bg-purple-500/10 hover:border-purple-500/20 hover:text-purple-400 hover:opacity-100 flex items-center justify-center shrink-0 transition-all">
              ...
              <div>
                ...
                <p className={cn("text-[11px] opacity-75 leading-relaxed", styles.textMutedClass)}>
                  Questions about your order or account? Message store owner directly on Instagram DM.
                </p>
              </div>
