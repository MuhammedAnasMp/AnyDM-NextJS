    ...
    if (username) {
      api.get(`/accounts/public/store/${username}/`).then((res) => {
        if (res.data?.settings) {
          ...
          setPrivacy(res.data.settings.privacy_policy || "We value your privacy. Your personal information is exclusively used to fulfill your orders.");
        }
      ...
      <div className="space-y-6">
        ...
        {loading ? (
          <div className="py-12 text-center text-zinc-500 text-sm flex items-center justify-center gap-2">
            <RefreshCw className="w-4 h-4 animate-spin" /> Loading Privacy Policy...
          </div>
        ) : (
