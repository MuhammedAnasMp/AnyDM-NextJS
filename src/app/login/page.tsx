...
function LoginContent() {
  ...
  const getFriendlyErrorMessage = (code: string) => {
    switch (code) {
      case "auth/popup-closed-by-user": return "The login popup was closed. Let's try again?";
      case "auth/user-not-found": return "We couldn't find an account with that email.";
      case "auth/wrong-password": return "That password doesn't seem right. Try again?";
      case "auth/invalid-email": return "That email looks a bit off. Could you double-check it?";
      case "auth/too-many-requests": return "Whoa, slow down! Too many attempts. Try again in a bit.";
      default: return "Something went wrong on our end. Could you try one more time?";
    }
  };
    ...
    <div className="min-h-screen bg-[#131313] text-[#e5e2e1] flex items-center justify-center p-6 relative overflow-hidden font-sans">
      ...
      >
        <div className="flex flex-col items-center text-center mb-8">
          ...
          <p className="text-sm text-[#c4c7c8]/60 mt-1">
            {isResetMode ? "Enter your email to reset password" : "Sign in to manage your social channels"}
          </p>
        </div>

        <form onSubmit={isResetMode ? (e) => e.preventDefault() : handleEmailLogin} className="space-y-4">
          <div>
          ...
          {!isResetMode && (
            <div>
              <div className="flex justify-between items-center mb-2">
                <label className="block text-xs font-bold uppercase tracking-wider text-[#c4c7c8]/80">Password</label>
                <button
                  type="button"
                  onClick={() => setIsResetMode(true)}
                  className="text-xs text-[#c4c7c8]/40 hover:text-white transition-colors"
                >
                  Forgot password?
                </button>
              </div>
          ...
          <button
            ...
            onClick={isResetMode ? handleForgotPassword : undefined}
            disabled={isAnyLoading}
            className="w-full bg-white text-black py-3 rounded-lg text-sm font-bold shadow-lg hover:bg-[#eaeaea] hover:scale-[1.01] active:scale-95 transition-all duration-200 flex items-center justify-center gap-2 mt-6 disabled:opacity-50 relative"
          >
            {isEmailLoading ? (
        ...
        {isResetMode && (
          <div className="text-center mt-4">
            <button
              ...
              className="text-xs text-[#c4c7c8]/60 hover:text-white transition-colors"
            >
              Back to Login
            </button>
          </div>
        ...
        <p className="text-center text-sm text-[#c4c7c8]/40 mt-8">
          ...
          <Link href="/signup" className="text-white font-semibold hover:underline">Sign up</Link>
        </p>
      </motion.div>
    </div>
  );
