...
export default function BioDashboardClient({
  initialTab = "styling",
}: {
  initialTab?: "blocks" | "styling" | "social" | "redirects" | "settings" | "analytics";
}) {
  ...
  const [activeTab, setActiveTabState] = useState<"blocks" | "styling" | "social" | "redirects" | "settings" | "analytics">(initialTab);

  // Helper function to set active tab and sync with URL routes (/dashboard/bio/styling vs /dashboard/bio/analytics)
  const setActiveTab = (newTab: "blocks" | "styling" | "social" | "redirects" | "settings" | "analytics") => {
    setActiveTabState(newTab);
    if (newTab === "analytics") {
      router.push("/dashboard/bio/analytics");
    } else if (newTab === "styling") {
      router.push("/dashboard/bio/styling");
    } else {
      router.push(`/dashboard/bio/styling?tab=${newTab}`);
    }
  };

  // Synchronize activeTab from route pathname and search parameters
  useEffect(() => {
    if (pathname?.includes("/dashboard/bio/analytics")) {
      setActiveTabState("analytics");
      return;
    }

    if (pathname?.includes("/dashboard/bio/styling")) {
      const tabParam = searchParams?.get("tab")?.toLowerCase();
      if (tabParam === "block" || tabParam === "blocks") {
  ...
  // Listen for mobile bottom nav CTA chip events (Preview & QR Code)
  useEffect(() => {
    const handlePreview = () => setMockPreviewModalOpen(true);
    const handleQr = () => setQrModalOpen(true);

    window.addEventListener("open-bio-preview-modal", handlePreview);
    window.addEventListener("open-bio-qr-modal", handleQr);

    return () => {
      window.removeEventListener("open-bio-preview-modal", handlePreview);
      window.removeEventListener("open-bio-qr-modal", handleQr);
    };
  }, []);
  ...
  const handleSavePageSettings = async (overrideData?: Partial<BioPageData>) => {
    const targetUsername = (overrideData?.username !== undefined ? overrideData.username : (usernameInput || page.username || "")).trim();
    if (!targetUsername) {
      showToast("Site name / Username cannot be empty", "error");
      return;
    }

    const validUsernameRegex = /^[a-zA-Z0-9.-]+$/;
    if (!validUsernameRegex.test(targetUsername)) {
      showToast("Site name can only contain letters (a-z, A-Z), numbers (0-9), hyphens (-), and dots (.)", "error");
      return;
    }
  ...
  return (
    ...
    >
      ...
      {/* Primary Page Header Bar (Matches /dashboard/automation layout & DESIGN.md) */}
      {activeTab !== "analytics" && (
        <>
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-[#1c1b1b] p-3 rounded border border-[#2a2a2a] shadow-sm">
            <div className="space-y-0.5">
              <div className="flex items-center justify-between gap-2 sm:justify-start">
                ...
                <button
                  ...
                  disabled={saving}
                  className="flex sm:hidden px-3 py-1 rounded bg-white hover:bg-zinc-200 text-zinc-950 font-bold text-xs items-center justify-center gap-1 transition-all cursor-pointer disabled:opacity-50 shrink-0 shadow-sm"
                >
                  {saving ? (
                    <>
              ...
              <p className="text-xs text-[#8e9192]">
                Customize your bio link page, manage custom blocks, social connections, and smart redirects.
              </p>
            </div>
            ...
            <div className="flex items-center gap-2 shrink-0">
              <button
                ...
                className="hidden sm:flex px-3 py-1.5 rounded bg-[#20201f] border border-[#2a2a2a] text-xs font-semibold text-[#8e9192] hover:text-white hover:border-[#444748] transition-colors items-center gap-1.5 cursor-pointer"
              ...
              <a
                ...
                className="hidden sm:flex px-3 py-1.5 rounded bg-[#20201f] border border-[#2a2a2a] text-xs font-semibold text-[#8e9192] hover:text-white hover:border-[#444748] transition-colors items-center gap-1.5"
              >
              ...
              <button
                ...
                onClick={() => handleSavePageSettings()}
                disabled={saving}
                className="hidden sm:flex px-4 py-2 bg-white hover:bg-zinc-200 text-zinc-950 text-xs font-semibold rounded items-center gap-1.5 transition-colors cursor-pointer disabled:opacity-50"
              >
                <Check className="w-3.5 h-3.5" />
          ...
          <div className="sticky top-[64px] z-30 flex flex-row items-center justify-between gap-2 bg-[#1c1b1b]/95 backdrop-blur-md p-2 sm:px-3 sm:py-2 rounded border border-[#2a2a2a] shadow-md">
            {/* Left Side: URL copy & Username Edit */}
            <div className="flex items-center gap-2 min-w-0 flex-1">
              <div className="w-6.5 h-6.5 rounded bg-[#20201f] border border-[#2a2a2a] flex items-center justify-center text-[#c4c0ff] shrink-0">
                <Link2 className="w-3.5 h-3.5" />
              </div>

              <div className="flex items-center gap-2 min-w-0 flex-1">
                {isEditingUsername ? (
                  <div className="flex items-center gap-1 bg-[#101010] border border-[#2a2a2a] focus-within:border-[#c4c0ff]/50 rounded px-2 py-0.5 text-xs text-white max-w-full">
                    <span className="text-[#8e9192] font-mono text-[11px] select-none shrink-0">{rootDomain}/</span>
                    <input
                      type="text"
                      value={usernameInput}
                      onChange={(e) => {
                      ...
                      className="bg-transparent text-white font-mono text-xs focus:outline-none w-20 sm:w-32 min-w-0"
                      autoFocus
                    />
                    <button
                      type="button"
                      ...
                      className="p-0.5 hover:text-[#c4c0ff] text-[#8e9192] transition-colors cursor-pointer shrink-0"
                      title="Save username"
                    >
                      <Check className="w-3.5 h-3.5 text-[#c4c0ff]" />
                    </button>
                    <button
                      type="button"
                      onClick={() => {
                      ...
                      className="p-0.5 hover:text-rose-400 text-[#8e9192] transition-colors cursor-pointer shrink-0"
                      title="Cancel"
                    >
                  ...
                  <div className="flex items-center gap-1.5 min-w-0 max-w-full">
                    <button
                      ...
                      className="flex items-center gap-1.5 px-2.5 py-1 rounded bg-[#20201f] hover:bg-[#2a2a2a] border border-[#2a2a2a] hover:border-[#444748] transition-all cursor-pointer select-none group/copy min-w-0"
                      title="Click to copy link"
                    >
                      <span className="text-xs font-semibold text-[#e5e2e1] tracking-tight truncate">
                        {rootDomain}/{page.username}
                      </span>
                      {copiedLink ? (
                        <Check className="w-3.5 h-3.5 text-[#c4c0ff] shrink-0" />
                      ) : (
                        <Copy className="w-3.5 h-3.5 text-[#8e9192] group-hover/copy:text-white transition-colors shrink-0" />
                      )}
                    </button>
                    <button
                      type="button"
                      onClick={() => setIsEditingUsername(true)}
                      className="p-1 rounded text-[#8e9192] hover:text-white hover:bg-[#20201f] transition-colors cursor-pointer shrink-0"
            ...
            <div className="flex items-center justify-end gap-2 shrink-0">
              <button
                ...
                className="flex items-center gap-2 text-xs font-semibold cursor-pointer select-none shrink-0 active:scale-95 text-white/90 hover:text-white"
              ...
              >
                ...
                <div
                  className={cn(
                    ...
                    page.is_published ? "bg-white" : "bg-white/20"
                  )}
                >
                  <div
                    className={cn(
                      "w-3 h-3 rounded-full transition-transform duration-200 shadow-sm",
                      page.is_published ? "bg-black translate-x-3" : "bg-white/60 translate-x-0"
                    )}
                  />
                </div>
              </button>
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 bg-[#1c1b1b] p-1.5 rounded border border-[#2a2a2a]">
            ...
            <div className="flex items-center gap-1 overflow-x-auto scrollbar-hide">
              ...
              <button
                ...
                className={cn(
                  "px-3 py-1.5 rounded text-xs font-semibold flex items-center gap-1.5 whitespace-nowrap transition-colors cursor-pointer shrink-0 select-none",
                  activeTab === "styling"
                    ? "bg-white text-zinc-950 font-bold shadow-xs"
                    : "bg-[#20201f] text-[#8e9192] hover:text-white border border-[#2a2a2a]"
                )}
              >
                {activeTab === "styling" && <Palette className="w-3.5 h-3.5 shrink-0 hidden sm:inline-block" />}
                <span>Styling</span>
              </button>

              {/* Drag & Reorderable Page Section Tabs */}
              <Reorder.Group
                axis="x"
                ...
                className="flex items-center gap-1 bg-[#101115] border border-[#2a2a2a] p-0.5 rounded"
              >
                  ...
                  return (
                    ...
                    >
                      <button
                        ...
                        className={cn(
                          "px-2 py-1 rounded text-xs font-semibold flex items-center gap-1 whitespace-nowrap transition-colors cursor-grab active:cursor-grabbing select-none",
                          isActive
                            ? "bg-white text-zinc-950 font-bold shadow-xs"
                            : "text-[#8e9192] hover:text-white"
                        )}
                        title="Drag to reorder section order on live bio page"
                      >
                        <GripVertical className="w-3 h-3 text-[#8e9192] shrink-0 opacity-60 hover:opacity-100" />
                        {isActive && <Icon className="w-3.5 h-3.5 shrink-0 hidden sm:inline-block" />}
            ...
            <div className="hidden lg:flex items-center gap-0.5 bg-[#101115] p-0.5 rounded border border-[#2a2a2a] shrink-0">
              <button
                type="button"
                onClick={() => setPreviewDevice("mobile")}
                className={cn(
                  "px-2.5 py-1 rounded text-xs font-semibold flex items-center gap-1.5 transition-colors select-none cursor-pointer",
                  previewDevice === "mobile" ? "bg-white text-zinc-950 font-bold shadow-xs" : "text-[#8e9192] hover:text-white"
                )}
              ...
              <button
                ...
                className={cn(
                  "px-2.5 py-1 rounded text-xs font-semibold flex items-center gap-1.5 transition-colors select-none cursor-pointer",
                  previewDevice === "tablet" ? "bg-white text-zinc-950 font-bold shadow-xs" : "text-[#8e9192] hover:text-white"
                )}
                title="iPad Preview"
              >
              ...
              <button
                ...
                className={cn(
                  "px-2.5 py-1 rounded text-xs font-semibold flex items-center gap-1.5 transition-colors select-none cursor-pointer",
                  previewDevice === "desktop" ? "bg-white text-zinc-950 font-bold shadow-xs" : "text-[#8e9192] hover:text-white"
                )}
                title="Mac Preview"
              >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-3 items-start">
        ...
        <div
          className={cn(
            "flex flex-col gap-3",
            activeTab === "analytics" ? "lg:col-span-12" : "lg:col-span-7"
          )}
        >
          {/* TAB 1: CONTENT BLOCKS */}
          {activeTab === "blocks" && (
            <div className="bg-[#1c1b1b] p-3.5 rounded border border-[#2a2a2a] space-y-3">
              <div className="flex items-center justify-between border-b border-white/5 pb-3">
                <h2 className="text-xs sm:text-sm font-semibold text-[#e5e2e1] flex items-center gap-2">
                  <span>Content Blocks</span>
                  <span className="px-1.5 py-0.5 rounded text-[10px] font-semibold bg-[#c4c0ff]/10 text-[#c4c0ff] border border-[#c4c0ff]/20">
                    {blocks.length}
                  </span>
                </h2>
                <div className="flex items-center gap-2">
                  ...
                  <button
                    ...
                    className="flex items-center gap-1.5 cursor-pointer select-none bg-[#20201f] px-2.5 py-1 rounded border border-[#2a2a2a] hover:border-[#444748] transition-all shrink-0 h-[28px]"
                  ...
                  >
                    ...
                    <div
                      className={cn(
                        ...
                        page.blocks_enabled !== false ? "bg-[#c4c0ff]" : "bg-[#2a2a2a]"
                      )}
                    >
                      <div
                        className={cn(
                          "w-2.5 h-2.5 rounded-full bg-[#131313] transition-transform duration-200 shadow-sm",
                  ...
                  <button
                    ...
                    className="px-2.5 py-1 bg-white text-zinc-950 font-semibold text-xs rounded hover:bg-zinc-200 transition-all flex items-center gap-1 cursor-pointer shadow-xs shrink-0 h-[28px] select-none"
                  >
              ...
              <div className="space-y-2">
                {blocks.length === 0 ? (
                  <div className="py-8 text-center rounded border border-dashed border-[#2a2a2a] p-5 space-y-2 bg-[#20201f]/50">
                    <Layers className="w-6 h-6 mx-auto text-[#8e9192]" />
                    <button
                      onClick={() => {
                        setEditingBlock(null);
                        setBlockModalOpen(true);
                      }}
                      className="px-3 py-1.5 bg-white text-zinc-950 font-semibold text-xs rounded hover:bg-zinc-200 transition-all cursor-pointer"
                    >
                      + Add Block
                    </button>
                  </div>
                    ...
                    {blocks.map((block, index) => {
                      ...
                      return (
                        <Reorder.Item
                          ...
                          className={cn(
                            "group rounded border transition-all select-none overflow-hidden cursor-pointer",
                            isCurrentlyEditing
                              ? "bg-[#20201f] border-[#c4c0ff] ring-1 ring-[#c4c0ff]/50 shadow-md shadow-[#c4c0ff]/10"
                              : block.is_active
                                ? "bg-[#20201f] border-[#2a2a2a] hover:border-[#444748] shadow-xs"
                          ...
                          <div className="flex items-center justify-between gap-2.5 p-2.5 sm:p-3 hover:bg-white/[0.02] transition-colors">
                            <div className="flex items-center gap-2 min-w-0 flex-1">
                              ...
                              <div
                                ...
                                className="p-1 rounded text-[#8e9192] hover:text-white hover:bg-white/10 cursor-grab active:cursor-grabbing transition-colors shrink-0"
                              ...
                              <div className="flex items-center gap-2 min-w-0 flex-1">
                                <h3 className="text-xs font-semibold text-[#e5e2e1] truncate">
                                  {block.title || "Untitled Block"}
                                </h3>
                                {block.url && (
                                  <span className="text-[10px] text-[#8e9192] truncate hidden sm:inline font-mono">
                                    {block.url}
                                  </span>
                                )}
                                {isCurrentlyEditing && (
                                  <span className="px-1.5 py-0.5 rounded text-[9px] font-semibold bg-[#c4c0ff] text-zinc-950 shrink-0 animate-pulse">
                                    Editing
                                  </span>
                                )}
                              </div>
                            ...
                            >
                              <button
                                ...
                                className={cn(
                                  ...
                                  block.is_active
                                    ? "text-[#c4c0ff] hover:bg-[#c4c0ff]/10"
                                    : "text-[#8e9192] hover:bg-white/10"
                                )}
                              ...
                              <button
                                ...
                                className="p-1.5 rounded text-[#8e9192] hover:text-white hover:bg-white/10 transition-colors cursor-pointer"
                                title="Edit block"
                              >
                              ...
                              <button
                                ...
                                className="p-1.5 rounded text-[#8e9192] hover:text-rose-400 hover:bg-rose-500/10 transition-colors cursor-pointer"
                                title="Delete block"
                              >
          ...
          {activeTab === "styling" && (
            <div className="bg-[#1c1b1b] p-3.5 rounded border border-[#2a2a2a] space-y-5 animate-in fade-in">
              {/* SECTION 1: PROFILE IDENTITY */}
              <div className="space-y-3">
                <div className="flex items-center gap-2 border-b border-white/5 pb-2">
                  <h2 className="text-xs sm:text-sm font-semibold text-[#e5e2e1] tracking-tight">Profile Details</h2>
                </div>

                {/* Media Cards: Avatar & Banner side by side */}
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {/* Profile Avatar Card */}
                  <div className="p-3 rounded bg-[#20201f] border border-[#2a2a2a] space-y-2.5">
                    <div className="flex items-center justify-between">
                      ...
                      {page.profile_image_url && (
                        <button
                          ...
                          className="text-[10px] font-semibold text-rose-400 hover:text-rose-300 transition-colors flex items-center gap-1 cursor-pointer"
                        >
                    ...
                    <div className="flex items-center gap-3">
                      <div className="w-12 h-12 rounded-full overflow-hidden border border-[#2a2a2a] shrink-0 bg-[#101115] flex items-center justify-center shadow-xs">
                        {page.profile_image_url || activeAccount?.profile_picture_url || appUser?.photo_url || appUser?.profile_picture_url ? (
                          ...
                          <Globe className="w-5 h-5 text-[#8e9192]" />
                        )}
                      </div>
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <button
                          ...
                          className="w-full py-1.5 px-3 rounded bg-[#101115] hover:bg-[#2a2a2a] border border-[#2a2a2a] text-xs font-semibold text-[#e5e2e1] flex items-center justify-center gap-1.5 transition-all cursor-pointer disabled:opacity-50 shadow-xs"
                        >
                          {avatarUploading ? (
                            <>
                            ...
                            <>
                              <Upload className="w-3.5 h-3.5 text-[#8e9192]" />
                  ...
                  <div className="p-3 rounded bg-[#20201f] border border-[#2a2a2a] space-y-2.5">
                    ...
                    <div className="flex items-center gap-3">
                      <div className="w-16 h-12 rounded overflow-hidden border border-[#2a2a2a] shrink-0 bg-[#101115] flex items-center justify-center shadow-xs">
                        {page.banner_image_url ? (
                          // eslint-disable-next-line @next/next/no-img-element
                          <img src={page.banner_image_url} alt="Banner" className="w-full h-full object-cover" />
                        ) : (
                          <ImageIcon className="w-5 h-5 text-[#8e9192]" />
                        )}
                      </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-1">
                  <div className="space-y-1.5">
                    <label className="text-xs font-semibold text-[#c4c7c8] flex items-center gap-1.5">
                      <Type className="w-3 h-3 text-[#8e9192]" />
                    ...
                    <input
                      ...
                      className="w-full bg-[#101115] border border-[#2a2a2a] focus:border-[#c4c0ff]/60 rounded px-3 py-2 text-xs text-[#e5e2e1] focus:outline-none transition-colors"
                    />
                  </div>

                  <div className="space-y-1.5">
                    <label className="text-xs font-semibold text-[#c4c7c8] flex items-center gap-1.5">
                      <AlignLeft className="w-3 h-3 text-[#8e9192]" />
                      <span>Bio</span>
                    </label>
                    <input
                      type="text"
              ...
              <div className="pt-2 border-t border-white/5 space-y-3">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-white/5 pb-2">
                  <div className="flex items-center gap-2">
                    ...
                    <h2 className="text-xs sm:text-sm font-semibold text-[#e5e2e1] tracking-tight">Appearance & Background</h2>
                  </div>

                  {/* Background Style Segmented Pill */}
                  <div className="inline-flex items-center gap-0.5 bg-[#101115] p-0.5 rounded border border-[#2a2a2a] self-start sm:self-auto">
                    <button
                      type="button"
                      onClick={() => {
                        setPage((prev) => ({
                      ...
                      className={cn(
                        "px-2.5 py-1 rounded text-[10px] sm:text-[11px] font-semibold flex items-center gap-1 transition-colors cursor-pointer select-none",
                ...
                {currentBgType === "preset" && (
                  <div className="p-3.5 rounded bg-[#20201f] border border-[#2a2a2a] space-y-2 animate-in fade-in">
                    <label className="text-xs font-semibold text-[#c4c7c8]">Preset Theme Selection</label>
                    ...
                    <div className="relative" ref={presetThemeRef}>
                      ...
                      <button
                        ...
                        className="w-full bg-[#101115] border border-[#2a2a2a] hover:border-[#444748] rounded p-2 text-xs text-[#e5e2e1] flex items-center justify-between transition-colors cursor-pointer select-none h-[36px]"
                      >
                        <div className="flex items-center gap-2 truncate min-w-0">
                          <div
                            className="w-5 h-4 rounded overflow-hidden border border-white/20 shrink-0 flex items-center justify-center p-0.5 shadow-xs"
                            style={{
                              background:
                          ...
                          >
                            <div
                              className="w-1.5 h-1.5 rounded-full shadow-xs ring-1 ring-black/40"
                              style={{
                                backgroundColor:
                          ...
                          <span className="font-semibold text-[#e5e2e1] truncate text-xs">
                            {(BIO_THEMES as Record<string, any>)[page.theme_id]?.name || "Select Theme"}
                          </span>
                        </div>
                        <ChevronDown className={cn("w-3.5 h-3.5 text-[#8e9192] transition-transform shrink-0 ml-1", presetThemeOpen && "rotate-180")} />
                      </button>

                      {/* Dropdown Menu */}
                      {presetThemeOpen && (
                        <div className="absolute top-full left-0 right-0 mt-1 bg-[#1c1b1b] border border-[#2a2a2a] rounded shadow-2xl z-30 p-1.5 space-y-1 max-h-64 overflow-y-auto custom-scrollbar">
                          {Object.values(BIO_THEMES).map((theme) => {
                            ...
                            return (
                              <button
                                ...
                                className={cn(
                                  "w-full px-2.5 py-2 rounded flex items-center justify-between text-left transition-colors cursor-pointer text-xs select-none",
                                  isSelected ? "bg-[#20201f] text-[#c4c0ff] font-semibold" : "text-[#c4c7c8] hover:bg-[#20201f] hover:text-white"
                                )}
                              >
                                <div className="flex items-center gap-2.5 min-w-0 flex-1">
                                  <div
                                    className="w-8 h-5.5 rounded overflow-hidden border border-white/20 shrink-0 flex items-center justify-center p-0.5 shadow-xs"
                                    style={{ background: gradientStyle }}
                                  >
                                    <div
                                      className="w-2 h-2 rounded-full shadow-xs ring-1 ring-black/40"
                ...
                {currentBgType === "color" && (
                  <div className="p-3.5 rounded-lg bg-[#20201f] border border-[#353535] space-y-2.5 animate-in fade-in">
                        ...
                        {PRESET_BG_PALETTES.map((palette) => {
                          ...
                          return (
                            <button
                              ...
                              className={cn(
                                "w-7 h-7 rounded-full border transition-all cursor-pointer flex items-center justify-center shadow-xs relative group shrink-0",
                                isChosen
                    ...
                    <div className="flex items-center gap-2 pt-1">
                      <div className="relative w-8 h-8 rounded overflow-hidden border border-[#2a2a2a] shrink-0 cursor-pointer shadow-xs">
                        <input
                          type="color"
                      ...
                      <input
                        ...
                        placeholder="#131313"
                        className="flex-1 bg-[#101115] border border-[#2a2a2a] focus:border-[#c4c0ff]/60 rounded px-2.5 py-1.5 text-xs text-[#e5e2e1] focus:outline-none font-mono"
                      />
                    </div>
                  </div>
                )}
                ...
                {currentBgType === "image" && (
                  <div className="p-3 rounded bg-[#20201f] border border-[#2a2a2a] space-y-3 sm:space-y-0 sm:flex sm:items-center sm:gap-3.5 animate-in fade-in">
                    {/* Left Column: Custom Wallpaper Upload */}
                    <div className="flex-1 space-y-1.5 min-w-0">
                      <div className="flex items-center justify-between">
                        <label className="text-xs font-semibold text-[#c4c7c8]">Custom Wallpaper</label>
                        {currentBgImage && (
                          <button
                            type="button"
                      ...
                      <div className="flex items-center gap-2">
                        <div className="w-12 h-9 rounded overflow-hidden border border-[#2a2a2a] shrink-0 bg-[#101115] flex items-center justify-center shadow-xs">
                          {currentBgImage ? (
                            // eslint-disable-next-line @next/next/no-img-element
                            <img src={currentBgImage} alt="Background" className="w-full h-full object-cover" />
                          ) : (
                            <ImageIcon className="w-4 h-4 text-[#8e9192]" />
                          )}
                        </div>
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <button
                            ...
                            className="w-full py-1.5 px-2.5 rounded bg-[#101115] hover:bg-[#2a2a2a] border border-[#2a2a2a] text-xs font-semibold text-[#e5e2e1] flex items-center justify-center gap-1.5 transition-all cursor-pointer disabled:opacity-50 shadow-xs"
                          >
                            {bgImageUploading ? (
                              <>
                              ...
                              <>
                                <Upload className="w-3 h-3 text-[#8e9192]" />
                                <span>Upload</span>
                              </>
                            )}
                          </button>
                    ...
                    {/* Divider Line */}
                    <div className="hidden sm:block w-px h-10 bg-[#2a2a2a] shrink-0" />

                    {/* Right Column: Overlay Dark Tint */}
                    <div className="flex-1 space-y-1.5 min-w-0">
                      <label className="text-xs font-semibold text-[#c4c7c8]">Overlay Dark Tint</label>
                      <div className="grid grid-cols-3 gap-1 bg-[#101115] p-1 rounded border border-[#2a2a2a]">
                        {[
                          { id: "light", label: "Soft" },
                          ...
                          return (
                            <button
                              ...
                              className={cn(
                                "py-1 px-1.5 rounded text-[10px] font-semibold text-center transition-colors cursor-pointer select-none truncate",
                                isSelected
                                  ? "bg-white text-zinc-950 font-bold shadow-xs"
                                  : "text-[#8e9192] hover:text-white"
                              )}
                ...
                <div className="p-3.5 rounded bg-[#20201f] border border-[#2a2a2a] space-y-2.5">
                  <div className="flex items-center justify-between">
                    <label className="text-xs font-semibold text-[#c4c7c8] flex items-center gap-1.5">
                    ...
                    {currentTextColor && (
                      <button
                        ...
                        className="text-[10px] text-[#8e9192] hover:text-white transition-colors flex items-center gap-1 cursor-pointer"
                    ...
                    {[
                      ...
                      return (
                        <button
                          ...
                          className={cn(
                            "w-6 h-6 rounded-full border transition-all cursor-pointer flex items-center justify-center shadow-xs shrink-0",
          ...
          {activeTab === "social" && (
            <div className="bg-[#1c1b1b] p-3.5 rounded border border-[#2a2a2a] space-y-3 ">
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2.5 border-b border-white/5 pb-3">
                {/* Label & Active Count */}
                <div className="flex items-center gap-2">
                  <h2 className="text-xs sm:text-sm font-semibold text-[#e5e2e1] tracking-tight flex items-center gap-2">
                ...
                <div className="flex flex-wrap items-center justify-end gap-2 ml-auto">
                  ...
                  >
                    ...
                    <div
                      className={cn(
                        "w-6 h-3.5 rounded-full p-0.5 transition-colors duration-200 flex items-center",
                        page.social_enabled !== false ? "bg-[#c4c0ff]" : "bg-[#2a2a2a]"
                      )}
                    >
                      <div
                        className={cn(
                  ...
                  <button
                    ...
                    className="px-2.5 py-1 rounded bg-white text-zinc-950 font-semibold text-xs hover:bg-zinc-200 transition-all flex items-center gap-1 cursor-pointer shadow-xs shrink-0 h-[28px] select-none"
                  >
                    <Plus className="w-3.5 h-3.5" />
                    <span>Add Link</span>
                  </button>
              ...
              <div className="space-y-1 relative" ref={popularNetworkRef}>
                ...
                <button
                  ...
                  className="w-full bg-[#101115] border border-[#2a2a2a] hover:border-[#444748] rounded p-2 text-xs text-[#e5e2e1] flex items-center justify-between transition-colors cursor-pointer select-none h-[36px]"
                >
                  <div className="flex items-center gap-2 text-[#8e9192] truncate min-w-0">
                    <Plus className="w-3.5 h-3.5 text-[#c4c0ff] shrink-0" />
                    <span className="font-medium text-xs text-[#8e9192] truncate">
                      Select network...
                    </span>
                  </div>
                  <ChevronDown
                    className={cn(
                      "w-3.5 h-3.5 text-[#8e9192] transition-transform shrink-0 ml-1",
              ...
              <div className="space-y-2 pt-2 border-t border-white/5">
                <div className="flex items-center justify-between">
                  <h3 className="text-xs font-semibold text-[#e5e2e1] tracking-wider">
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      ...
                      className="flex items-center gap-1.5 cursor-pointer select-none px-2 py-1 rounded border border-[#2a2a2a] bg-[#20201f] hover:border-[#444748] transition-all"
                    >
                      <span className="text-xs font-semibold text-[#8e9192]">Labels</span>
                      <div
                        className={cn(
                          "w-6 h-3.5 rounded-full p-0.5 transition-colors duration-200 flex items-center",
                          page.show_social_usernames ? "bg-[#c4c0ff]" : "bg-[#2a2a2a]"
                        )}
                      >
                        <div
                          className={cn(
                ...
                {(page.social_accounts || []).length === 0 ||
                  ...
                  <div className="py-3.5 text-center rounded border border-dashed border-[#2a2a2a] p-3 bg-[#20201f]/50">
                    <p className="text-xs font-medium text-[#8e9192]">No active social links</p>
                  </div>
                ) : (
                  <Reorder.Group
                    axis="y"
                        ...
                        return (
                          <Reorder.Item
                            ...
                            className={cn(
                              ...
                              account.is_active
                                ? "border-[#2a2a2a] hover:border-[#444748] shadow-xs"
                                : "bg-[#1c1b1b] border-[#2a2a2a] opacity-60"
                            )}
                          >
                            {/* Drag Handle */}
                            <div className="cursor-grab active:cursor-grabbing p-0.5 text-[#8e9192] hover:text-white shrink-0">
                              <GripVertical className="w-4 h-4" />
                            </div>
                            ...
                            <div
                              className={cn(
                                ...
                                account.is_active ? "bg-white/10 text-white" : "bg-white/5 text-[#8e9192]"
                              )}
                            >
                            ...
                            {isEditing ? (
                              <>
                                ...
                                <input
                                  ...
                                  placeholder="Title"
                                  className="w-20 sm:w-28 bg-[#101115] border border-[#2a2a2a] focus:border-[#c4c0ff]/60 rounded px-2 py-1 text-xs text-[#e5e2e1] placeholder:text-[#8e9192] focus:outline-none transition-colors shrink-0"
                                />
                                ...
                                <input
                                  ...
                                  className="flex-1 min-w-0 bg-[#101115] border border-[#2a2a2a] focus:border-[#c4c0ff]/60 rounded px-2.5 py-1.5 text-xs text-[#e5e2e1] placeholder:text-[#8e9192] focus:outline-none disabled:opacity-30 disabled:cursor-not-allowed font-mono transition-colors"
                                  autoFocus
                                />
                              </>
                            ) : (
                              <>
                                ...
                                <div className="w-20 sm:w-24 shrink-0 truncate">
                                  <span className="text-xs font-semibold text-[#e5e2e1] truncate block">
                                ...
                                <div
                                  ...
                                  className="flex-1 min-w-0 bg-[#101115] border border-[#2a2a2a] hover:border-[#444748] rounded px-2.5 py-1.5 text-xs font-mono transition-colors truncate cursor-pointer select-none"
                                  title="Click to edit link"
                                >
                                  {account.url ? (
                                    <span className="text-[#c4c7c8]">{account.url}</span>
                                  ) : (
                                    <span className="text-[#8e9192] italic">Click Edit to add link</span>
                                  )}
                                </div>
                              </>
                            )}
                            ...
                            <div className="flex items-center gap-1 shrink-0" onPointerDown={(e) => e.stopPropagation()}>
                              {isEditing ? (
                                <button
                                  ...
                                  className="p-1.5 rounded bg-[#c4c0ff]/20 text-[#c4c0ff] hover:bg-[#c4c0ff]/30 transition-colors cursor-pointer"
                                  title="Done editing"
                                >
                                  <Check className="w-3.5 h-3.5" />
                                </button>
                                ...
                                <button
                                  ...
                                  className="p-1.5 rounded hover:bg-white/10 text-[#8e9192] hover:text-white transition-colors cursor-pointer"
                                  title="Edit link"
                                >
                                  <Edit3 className="w-3.5 h-3.5" />
                                </button>
                              ...
                              <input
                                ...
                                className="w-3.5 h-3.5 accent-[#c4c0ff] cursor-pointer mr-0.5"
                                title="Toggle active"
                              />
                              ...
                              <button
                                ...
                                className="p-1.5 rounded hover:bg-rose-500/10 text-[#8e9192] hover:text-rose-400 transition-colors cursor-pointer"
                                title="Remove link"
                              >
                                <Trash2 className="w-3.5 h-3.5" />
                              </button>
          ...
          {activeTab === "redirects" && (
            <div className="bg-[#1c1b1b] p-3.5 rounded border border-[#2a2a2a] space-y-3 ">
                ...
                <div className="flex items-center gap-2">
                  ...
                  >
                    ...
                    <div
                      className={cn(
                        "w-6 h-3.5 rounded-full p-0.5 transition-colors duration-200 flex items-center",
                        page.smart_redirect_enabled !== false ? "bg-[#c4c0ff]" : "bg-[#2a2a2a]"
                      )}
                    >
                      <div
                        className={cn(
              ...
              {page.smart_redirect_enabled !== false && (
                <div className="p-3.5 rounded bg-[#20201f] border border-[#2a2a2a] space-y-3 animate-in fade-in">
                  <div className="flex items-center justify-between border-b border-white/5 pb-2">
                    <label className="text-xs font-semibold text-[#c4c7c8]">Card Text & Custom Labels</label>
                    <span className="text-[10px] text-[#8e9192]">Live preview sync</span>
                  </div>
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-2.5">
                    <div className="space-y-1">
                      <label className="text-[11px] text-[#8e9192] font-medium">Card Title</label>
                      <input
                        type="text"
                        ...
                        className="w-full bg-[#101115] border border-[#2a2a2a] focus:border-[#c4c0ff]/60 rounded px-2.5 py-1.5 text-xs text-[#e5e2e1] focus:outline-none transition-colors"
                      />
                    </div>

                    <div className="space-y-1">
                      <label className="text-[11px] text-[#8e9192] font-medium">Input Placeholder</label>
                      <input
                        type="text"
                    ...
                    <div className="space-y-1">
                      <label className="text-[11px] text-[#8e9192] font-medium">Button Label</label>
                      <div
                        className={cn(
                          "w-full rounded px-3 py-1.5 flex items-center justify-center gap-2 transition-all shadow-xs border border-[#2a2a2a] focus-within:border-[#c4c0ff]/60 cursor-text",
                      ...
                      >
                        <input
                          ...
                          className="bg-transparent text-center focus:outline-none placeholder:opacity-60 text-xs font-semibold text-current max-w-[85%]"
                        />
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  <h3 className="text-xs font-semibold text-[#e5e2e1] uppercase tracking-wider">
                    Configured Rules ({redirectRules.length})
                  </h3>
                </div>

                {redirectRules.length === 0 ? (
                  <div className="py-6 text-center rounded border border-dashed border-[#2a2a2a] p-4 bg-[#20201f]/50">
                    <p className="text-xs text-[#8e9192]">No redirect rules set up</p>
                  </div>
                ) : (
                  redirectRules.map((rule) => {
                    ...
                    return (
                      <div
                        key={rule.id}
                        className="group p-2.5 rounded bg-[#20201f] border border-[#2a2a2a] hover:border-[#444748] transition-colors flex items-center justify-between gap-3 select-none"
                      >
                        ...
                        <div className="flex items-center gap-2.5 min-w-0 flex-1">
                          ...
                          <div className="flex items-center gap-2 text-xs truncate min-w-0 font-mono">
                            <span className="font-semibold text-[#e5e2e1] font-sans shrink-0 truncate max-w-[150px]">
                              {rule.title || "Rule"}
                            </span>
                            <span className="text-zinc-600 font-sans shrink-0">•</span>
                            <span className="text-[#8e9192] truncate max-w-[200px]" title={rule.input_match_url}>
                              {rule.input_match_url}
                            </span>
                            <span className="text-zinc-500 shrink-0">➔</span>
                            <span className="text-[#c4c7c8] font-semibold truncate max-w-[220px]" title={rule.destination_value}>
                        ...
                        <div className="flex items-center gap-1.5 shrink-0">
                          ...
                          <button
                            ...
                            className="px-2.5 py-1 rounded bg-[#20201f] border border-[#2a2a2a] hover:bg-white text-[#e5e2e1] hover:text-zinc-950 text-xs font-semibold flex items-center gap-1 transition-all cursor-pointer shadow-xs"
                            title={targetRedirectUrl !== "#" ? `Test redirect to ${targetRedirectUrl}` : "No valid URL to test"}
                          >
                            <Play className="w-3 h-3 fill-current text-[#c4c0ff] group-hover:text-zinc-950" />
                            <span>Test</span>
                          </button>
                          ...
                          <button
                            ...
                            className="p-1.5 rounded text-[#8e9192] hover:text-white hover:bg-white/10 cursor-pointer transition-colors"
                            title="Edit rule"
                          >
                            <Edit3 className="w-3.5 h-3.5" />
                          </button>
                          <button
                            ...
                            className="p-1.5 rounded text-[#8e9192] hover:text-rose-400 hover:bg-rose-500/10 cursor-pointer transition-colors"
                            title="Delete rule"
                          >
                            <Trash2 className="w-3.5 h-3.5" />
                          </button>
          ...
          {activeTab === "analytics" && (() => {
            ...
            return (
              <div className="bg-[#1c1b1b] p-4 rounded border border-[#2a2a2a] shadow-xs space-y-4">
                {/* Header aligned with DESIGN.md Surface Container Low */}
                <div className="flex items-center justify-between gap-2 border-b border-white/5 pb-3">
                  <div className="flex items-center gap-2 sm:gap-2.5 min-w-0">
                    <div className="w-7 h-7 sm:w-8 sm:h-8 rounded bg-[#c4c0ff]/10 flex items-center justify-center text-[#c4c0ff] shrink-0">
                      <BarChart3 className="w-3.5 h-3.5 sm:w-4 sm:h-4" />
                    </div>
                    <h2 className="text-xs sm:text-base font-semibold text-[#e5e2e1] tracking-tight truncate">
                      Analytics &amp; Button Performance
                    </h2>
                  </div>

                  <div className="flex items-center gap-2 shrink-0">
                    <span
                      className="text-[10px] sm:text-xs font-semibold text-[#e5e2e1] bg-[#20201f] border border-[#2a2a2a] px-2 py-0.5 sm:px-2.5 sm:py-1 rounded flex items-center gap-1 sm:gap-1.5 select-none"
                      title="Click-Through Rate (CTR) = (Total Clicks / Total Page Views) × 100"
                    >
                      <div className="w-1.5 h-1.5 rounded-full bg-[#c4c0ff] animate-pulse" />
                      <span className="hidden sm:inline">Click-Through Rate:</span>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-5 gap-2.5 sm:gap-3">
                  {[
                    { label: "Total Page Views", value: totalViews.toLocaleString(), icon: Globe, color: "text-[#8e9192]" },
                    { label: "Total Link Clicks", value: totalClicks.toLocaleString(), icon: ExternalLink, color: "text-[#8e9192]" },
                    { label: "Click-Through Rate", value: `${ctr}%`, icon: BarChart3, color: "text-[#c4c0ff]" },
                    { label: "Smart Redirect Hits", value: totalRedirectHits.toLocaleString(), icon: Sparkles, color: "text-[#c4c0ff]" },
                    { label: "Tracked Buttons", value: allItems.length.toLocaleString(), icon: Layers, color: "text-[#8e9192]" },
                  ].map((stat, i) => {
                    const Icon = stat.icon;
                    return (
                      <div key={i} className="p-3 sm:p-3.5 rounded bg-[#20201f] border border-[#2a2a2a] space-y-1.5 transition-all hover:border-[#444748]">
                        <div className="flex items-center justify-between text-[#c4c7c8]">
                          <span className="text-[11px] font-semibold uppercase tracking-wider text-[#8e9192]">{stat.label}</span>
                ...
                <div className="space-y-3 pt-3 border-t border-white/5">
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                    ...
                    <div className="flex items-center gap-1 overflow-x-auto scrollbar-hide bg-[#101115] p-1 rounded border border-[#2a2a2a]">
                      {[
                        ...
                        <button
                          ...
                          className={cn(
                            "px-2.5 py-1 rounded text-xs font-semibold flex items-center gap-1.5 transition-all cursor-pointer whitespace-nowrap select-none",
                        ...
                        >
                          <span>{tab.label}</span>
                          <span className={cn("px-1.5 py-0.2 rounded text-[10px]", analyticsCategory === tab.key ? "bg-zinc-950/10 text-zinc-950 font-bold" : "bg-white/10 text-[#c4c7c8]")}>
                            {tab.count}
                          </span>
                        </button>
                      ))}
                    ...
                    <div className="relative w-full sm:w-64">
                      ...
                      <input
                        ...
                        className="w-full bg-[#101115] border border-[#2a2a2a] focus:border-[#c4c0ff]/60 rounded pl-8 pr-3 py-1.5 text-xs text-[#e5e2e1] placeholder:text-[#8e9192] focus:outline-none transition-colors"
                      />
                    </div>
                  </div>
                  ...
                  <div className="space-y-2 pt-1">
                    {filteredItems.length === 0 ? (
                      <div className="p-8 text-center bg-[#20201f]/50 rounded border border-[#2a2a2a] space-y-2">
                      ...
                      filteredItems.map((item, index) => {
                        ...
                        return (
                          <div
                            key={item.id}
                            className="group relative bg-[#20201f] hover:bg-[#2a2a2a] p-3 rounded border border-[#2a2a2a] hover:border-[#444748] transition-all space-y-2 shadow-xs overflow-hidden"
                          >
                            <div className="flex items-center justify-between gap-3 text-xs relative z-10">
                              ...
                              <div className="flex items-center gap-2.5 min-w-0 flex-1">
                                ...
                                <div className="w-7 h-7 rounded bg-white/5 border border-white/10 flex items-center justify-center shrink-0 text-white">
                                  {item.type === "block" && (
                                    <>
                                ...
                                <div className="min-w-0 flex-1">
                                  <div className="flex items-center gap-2">
                                    ...
                                    {isTopPerformer && (
                                      <span className="px-1.5 py-0.2 rounded text-[9px] font-semibold bg-amber-500/10 text-amber-300 border border-amber-500/20 flex items-center gap-0.5 shrink-0">
        ...
        {/* Right Column: Interactive Multi-Device Mockup Preview (Desktop only >= lg) */}
        {activeTab !== "analytics" && (
          <div className="hidden lg:flex flex-col sticky top-[128px] lg:col-span-5">
            <div className="w-full flex flex-col items-center justify-center bg-[#09090b] p-2 sm:p-3 rounded border border-[#20201f] shadow-2xl relative min-h-[470px] group/mockup">
              ...
              {previewDevice === "mobile" && (
                <div key={`mobile-${previewKey}`} className="w-full flex justify-center py-1">
              ...
              {previewDevice === "tablet" && (
                <div key={`tablet-${previewKey}`} className="w-full flex justify-center py-1">
              ...
              {previewDevice === "desktop" && (
                <div key={`desktop-${previewKey}`} className="w-full flex justify-center py-1">
