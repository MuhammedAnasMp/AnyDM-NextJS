...
export default function InboxPage() {
  ...
  useEffect(() => {
    ...
    if (currentParam !== String(selectedConversation.recipient_id)) {
      ...
      router.replace(`/dashboard/inbox/chats?${params.toString()}`, { scroll: false });
    }
  ...
  useEffect(() => {
    ...
    if (existing) {
      ...
      // While conversations are loading or on account change, wait for fetchConversations unless explicit name params exist
      if (loading && !usernameParam && !nameParam) return;

      const decodedAvatar = avatarParam ? decodeURIComponent(avatarParam) : "";
      const tempConv = {
        id: `temp_${recipientIdParam}`,
        name: usernameParam || nameParam || "User",
  ...
  }, [recipientIdParam, usernameParam, nameParam, avatarParam, conversations.length, loading]);

  useEffect(() => {
    window.dispatchEvent(new CustomEvent("mobile-chat-active", { detail: { isActive: showChatOnMobile && !!selectedConversation } }));
    return () => {
      window.dispatchEvent(new CustomEvent("mobile-chat-active", { detail: { isActive: false } }));
    };
  ...
  useEffect(() => {
    const handleRefresh = () => {
      fetchConversations();
      if (selectedConversationRef.current) fetchMessages(selectedConversationRef.current.id);
    };
      ...
      try {
        ...
        if (data.event_type === "new_message") {
          ...
          const formattedMessage = {
            id: message.id,
            sender: message.from?.username || (isSelf ? "You" : "User"),
    ...
    try {
      ...
      if (res.data.conversations) {
        ...
        if (selectedConversationIdRef.current?.startsWith("temp_")) {
          const tempConv = selectedConversationRef.current;
          const realConv = mapped.find((c: any) => String(c.recipient_id) === String(tempConv?.recipient_id));
          if (realConv) {
            setSelectedConversation(realConv);
          } else if (!tempConv?.name || tempConv?.name === "User" || tempConv?.name === "Instagram User") {
    ...
    try {
      ...
      if (res.data.messages) {
        const mapped = res.data.messages.map((m: any) => {
          ...
          return {
            id: m.id,
            sender: m.from?.username || (isSelf ? "You" : "User"),
            text: m.message || "",
            time: m.created_time
    ...
    >
        ...
        <div className="px-4 pt-4 pb-2 flex items-center justify-between shrink-0">
          ...
          <div className="flex items-center gap-1.5">
            <button
              ...
              disabled={loading}
              className="hidden sm:flex w-7 h-7 rounded-lg bg-white/5 hover:bg-white/10 border border-white/[0.07] items-center justify-center text-white/50 hover:text-white transition-all active:scale-95 disabled:opacity-40"
      ...
      <section className={`flex-1 flex flex-col min-w-0 bg-[#0d0d0d] ${showChatOnMobile ? "flex" : "hidden lg:flex"}`}>
        {selectedConversation ? (
          ...
          <div className="flex-1 flex flex-col justify-between min-h-0 bg-[#0d0d0d] animate-pulse">
            {/* Skeleton Header */}
            <div className="flex items-center justify-between px-4 py-3 border-b border-white/[0.06] bg-[#111]/80 shrink-0">
              <div className="flex items-center gap-2.5">
            ...
            <div className="flex-1 p-4 space-y-4 overflow-hidden">
              <div className="flex items-start gap-2 max-w-[70%]">
                ...
                <div className="h-10 w-48 rounded-2xl rounded-bl-sm bg-white/[0.04]" />
              </div>
              <div className="flex items-end justify-end gap-2 ml-auto max-w-[70%]">
                <div className="h-12 w-56 rounded-2xl rounded-br-sm bg-white/[0.08]" />
              </div>
              <div className="flex items-start gap-2 max-w-[70%]">
                <div className="w-7 h-7 rounded-full bg-white/10 shrink-0" />
                <div className="h-14 w-64 rounded-2xl rounded-bl-sm bg-white/[0.04]" />
              </div>
              <div className="flex items-end justify-end gap-2 ml-auto max-w-[70%]">
                <div className="h-10 w-40 rounded-2xl rounded-br-sm bg-white/[0.08]" />
              </div>
            </div>

            {/* Skeleton Footer Input */}
            <div className="p-3 border-t border-white/[0.06] bg-[#111]/80 shrink-0">
              <div className="h-11 w-full rounded-xl bg-white/[0.04] border border-white/5" />
            </div>
          </div>
        ) : (
