...
export default function TriggerContentEditor({ nodeId, onClose }: TriggerContentEditorProps) {
  ...
  return createPortal(
    <div className="fixed inset-0 z-[99999] flex items-center justify-center p-3 sm:p-4 bg-black/60 backdrop-blur-sm overflow-hidden text-white font-inter">
      ...
      {/* Main Modal Window */}
      <div className="w-full max-w-2xl max-h-[92vh] sm:max-h-[85vh] bg-[#131313]/95 backdrop-blur-3xl border border-white/10 rounded-xl overflow-hidden flex flex-col shadow-[0_32px_64px_rgba(0,0,0,0.6)] animate-in fade-in zoom-in duration-200 text-white relative z-10 font-inter">

        {/* Modal Header */}
        <div className="px-4 sm:px-5 py-3.5 sm:py-4 border-b border-white/10 flex flex-col sm:flex-row sm:items-center justify-between gap-3 shrink-0 bg-transparent">
          <div className="flex items-center gap-2.5 sm:gap-3">
            <div className="p-1.5 sm:p-2 bg-white/5 rounded-lg border border-white/10 shrink-0">
              <ArrowRightFromLine className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-white" />
            </div>
            <div>
              <h2 className="font-inter text-xs sm:text-sm md:text-base font-bold text-white tracking-tight leading-tight">
                Trigger Configuration
              </h2>
              <span className="text-[10px] sm:text-[11px] text-zinc-400 font-medium tracking-wide block mt-0.5 leading-tight">
        ...
        {validationError && (
          <div className="px-4 sm:px-5 py-2.5 bg-rose-500/10 border-b border-rose-500/20 flex items-center justify-between text-rose-400 text-xs font-semibold shrink-0 animate-in fade-in slide-in-from-top-1">
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-5 space-y-4 sm:space-y-5 custom-scrollbar bg-[#131313]">
          ...
          {isStandardDMRule ? (
            <div className="p-4 sm:p-5 rounded-xl bg-white/5 border border-white/10 space-y-3">
              <div className="flex items-center gap-3.5">
                ...
                <div>
                  <h4 className="text-xs sm:text-sm font-bold text-white font-inter">All Incoming Direct Messages</h4>
              ...
              <div className="space-y-2.5">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  ...
                  <button
                    ...
                    className={cn(
                      "p-4 rounded-xl border text-left transition-all cursor-pointer flex flex-col justify-between relative min-h-[110px]",
                      targetMode === 'every'
                ...
                <div className="p-4 rounded-xl bg-white/5 border border-white/10 space-y-3 shadow-sm animate-in fade-in duration-200">
                  <div className="flex items-center justify-between">
                    ...
                    <button
                      type="button"
                      onClick={() => setShowMediaPicker(true)}
                      className="px-3 sm:px-4 py-1.5 sm:py-2 rounded bg-white text-black font-bold text-xs flex items-center gap-1.5 hover:opacity-90 transition-all shadow-md active:scale-95 cursor-pointer"
                    >
                      ...
                      {selectedMediaDetails.map((media: any) => (
                        <div key={media.id} className="relative aspect-square rounded-lg bg-black border border-white/10 overflow-hidden group shadow-md">
                          ...
                          <button
                            ...
                            className="absolute top-1 right-1 p-1 rounded bg-black/80 hover:bg-rose-600 text-white transition-colors opacity-100 sm:opacity-0 sm:group-hover:opacity-100 cursor-pointer"
                            title="Remove"
                          >
